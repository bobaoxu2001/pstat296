...
ts.plot(percentage, type="l", main= "Original Data of Govt_Yield_20_ear" )
```
...
hist(c_train.bc, col = "light blue", xlab = "", main = "Histogram of train.bc ")
qqnorm(c_train.bc, main = "", xlab = "")
qqline(c_train.bc, col = "red")
```
```{r}
hist(c_train.bc, col = "light blue", xlab = "", main = "Histogram of Box-Cox Transformed Data ")
```
...
# non-gaussian distribution - long tail not perform well
# box-cox jenkins methodolgies 
# why other models - because of non-gaussian distribution
# 
```
...
par(mfrow=c(1, 1))
ts.plot(c_train.bc.1, main="De-trended Time Series",
...
col = "blue", xlab = "", prob = TRUE, main = "Residual Histogran") 
m1 <- mean(res1)
std1 <- sqrt(var(res1)) 
...
# Heavy long tails; Gaussian distribution didn't exist
# Simple mod
```
```{r}
...
```{r}
arima(c_train.bc, order = c(2,1,2),fixed=c(NA,NA,NA,0), method = "ML")
```
```{r}
AICc(arima(c_train.bc, order=c(2,1,2), method="ML")) 
```

$ (1+0.7102B)Y_t = (1 – 0.6048B)Z_t$ 

$ Y_t+0.3635Y_{t-1}+0.1216Y_{t-2} = Z_t + 0.4271Z_{t-1}$
$ Z_t  N(0,0.03777)$
```{r}
...
col = "blue", xlab = "", prob = TRUE, main = "Q-Q Normal Plot of Residual Points") 
m2 <- mean(res2)
std2 <- sqrt(var(res2)) 
curve(dnorm(x,m2,std2),add = TRUE)
qqnorm(res2,main = "Q-Q Normal Plot of Residual Points", xlab = "") 
...
plot.ts(c_train, xlim = c(2000,2023), ylim = c(0,8)) 
...
plot.ts(c_test, xlim = c(2022,2023), ylim = c(0,8), main = "Comparison of Prediction Points with Test Set")
lines(U.tr, col = "blue", lty = "dashed")
lines(L.tr, col = "blue", lty = "dashed")
points((lambda*pred.tr$pred + 1)^(1/lambda),col = "red", pch = 1) 
points(c_test,col = "slategrey", pch = 20)
