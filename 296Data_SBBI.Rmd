...
```
ACF outside confidence intervals: Lags 1, may be 3
PACF outside confidence intervals: Lags 1, 2, 3, 4
It seems like a MA model.
...
c12 <- AICc(arima(c_train.bc, order = c(2,1,2),
  seasonal = list(order = c(2,1,3), period = 12), method = "ML"))
```
```{r}
...
```{r}
# Plot the residuals
fit1 <- arima(c_train.bc, order = c(2,1,0),
              seasonal = list(order = c(0,1,1), period = 12), method = "ML") 
res1 <- residuals(fit1)
plot.ts(res1, xlab = "")
