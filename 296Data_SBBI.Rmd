...
train <- Govt_20_Yield$yield_percentage[709:1149]
test <- Govt_20_Yield$yield_percentage[1153:1161]
#train set contains data points from 1985.01 to 2018.12, 441 points totally
#test set contains data points from 2022.1 and 2022.9, 12 points totally
```
...
```
```{r}
arima(apt.log, order=c(0,1,3), seasonal = list(order = c(0,1,1), period = 12), fixed = c(NA, 0, NA, NA),
method="ML")
```
...
# Zoom in graph
par(mfrow=c(1,2))
plot.ts(tsdata, xlim = c(2021,2022), ylim = c(1,5))
lines(U.tr, col = "blue", lty = "dashed")
lines(L.tr, col = "blue", lty = "dashed") 
points(pred.tr$pred,col = "red", pch = 1) 
```
